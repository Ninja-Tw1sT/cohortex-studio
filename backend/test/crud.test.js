...
describe("agents CRUD", () => {
  test("create returns 201 with id, then lists", async () => {
  ...
  test("rejects an invalid backend enum with 400", async () => {
    const r = await request(app).post("/api/agents").send({ name: "x", backend: "not-a-backend" });
    expect(r.status).toBe(400);
    expect(r.body.error).toBeDefined();
  });

  test("rejects a missing name with 400", async () => {
    const r = await request(app).post("/api/agents").send({ role: "x" });
    expect(r.status).toBe(400);
  });

  test("duplicate (owner,name) returns 409", async () => {
    await request(app).post("/api/agents").send({ name: "dup" });
    const r = await request(app).post("/api/agents").send({ name: "dup" });
    expect(r.status).toBe(409);
  });

  test("update then delete round-trips", async () => {
    const created = await request(app).post("/api/agents").send({ name: "a", temperature: 0.1 });
    const { id } = created.body;

    const updated = await request(app).put(`/api/agents/${id}`).send({ temperature: 0.9 });
    expect(updated.body.temperature).toBe(0.9);

    const deleted = await request(app).delete(`/api/agents/${id}`);
    expect(deleted.body.ok).toBe(true);

    const gone = await request(app).get(`/api/agents/${id}`);
...
describe("crews CRUD", () => {
  ...
  test("rejects an invalid topology with 400", async () => {
    const r = await request(app).post("/api/crews").send({ name: "x", topology: "mesh" });
    expect(r.status).toBe(400);
  });
});
